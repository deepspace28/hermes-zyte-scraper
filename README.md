...
- Modern `ctx.register_tool(schema, handler)` + `ctx.register_hook` / `ctx.register_command` / `ctx.register_cli_command` / `ctx.register_skill`.
...
See the internal Zyte study for exhaustive subsection mapping.
...
As of 2026, the plugin implements the core vision at high quality:
...
Further production hardening (full custom attributes + sessions exposure, expanded schedule CRUD, richer cost tooling) is welcome.
